...
class TokenioNoticeService
{
     ...
     * @throws \Throwable
     */
    public function processRecharge(CoinTokenioNotice $notice, array $options = [])
    {
        ...
        $symbolArr = explode('.', $result['symbol']);
        if (count($symbolArr) > 1) {
            //出现的币种如：TRC20.USDT
            $coin = $coinService->getBySymbol($symbolArr[1]);
        } else {
     ...
     * @param array $options
     */
    public function processWithdraw(CoinTokenioNotice $notice, array $options = [])
    {
