...
@push('after-scripts')
    ...
    <script>
        layui.use(['form', 'table', 'util', 'laydate'], function () {
            ...
            table.render({
                elem: '#lay-table',
                url: '{{ route('m.dsy.api.admin.api.unite.index') }}',
            ...
            table.on("tool(lay-table)", function (e) {
                ...
                if (e.event === 'edit_user') {
                    var url = '{{ route('m.dsy.admin.unite.edit') }}?id=' + data.id;
                    layer.open({
                        type: 2
                        , title: '编辑：' + data['id'] + "信息"
                        , content: url
                ...
                if (e.event==='delete'){
                    var urls='{{route('m.dsy.api.admin.api.unite.del')}}';
                    $.ajax({
                        url: urls,
                        dataType: 'json',
                        type: 'post',
            ...
            let active = {
                ...
                create: function () {
                    ...
                    var url = '{{ route('m.dsy.admin.unite.create') }}';
                    layer.open({
                        type: 2
                        , title: "添加产品"
                        , content: url
