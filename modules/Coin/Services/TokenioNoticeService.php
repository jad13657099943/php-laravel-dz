...
class TokenioNoticeService
{
    ...
    {
        $service = resolve(\Modules\Coinv2\Services\TokenioNoticeService::class);
        if ($notice->needProcess() || ($options['force'] ?? true)) {
            try {
                if ($notice->type='recharge') {
                    $service->processRecharge($notice, $options['rechargeOptions'] ?? []);
                } elseif ($notice->type == 'withdraw') {
                    $service->processWithdraw($notice, $options['withdrawOptions'] ?? []);
                } else {
