...
class WithdrawService
{
    ...
    protected function creating(array $data, array $options = [])
    {
       ...
       // $cost = $coin->calcWithdrawFeeWith($data['num']);
        $fee= Price::query()->where('symbol',$data['symbol'])->value('free');
        $cost['fee']=$fee;
        $cost['num']=$data['num'];
        //判断该用户币种资产释放单独冻结
        ...
        if ($balance<$data['num']+$fee) throw new \Exception(trans('dsy::message.手续费不足'));
        ...
        if ($options['changeBalanceOptions'] ?? true) { // 修改余额
            $free= ['id' => $model->user_id, 'symbol' => $data['symbol'], 'num' => $fee, 'message' => 'dsy::message.提现手续费扣除'];
            free($free);
